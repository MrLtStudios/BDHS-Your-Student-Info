(() => {
  ...
  function bindAuth(m){
    if(m==='login'||m==='signup'){
      ...
      $('#google').onclick=()=>{try{const id=window.BDHS_GOOGLE_CLIENT_ID;if(!id||id.includes('YOUR_GOOGLE_CLIENT_ID'))throw Error('Google sign-in is not configured yet.');if(!window.google?.accounts?.id)throw Error('Google is still loading. Try again in a moment.');if(!window.__bdhsGoogleInitialized){window.google.accounts.id.initialize({client_id:id,callback:async r=>{try{await BDHSAuth.google(r.credential,$('#remember').checked);showDash()}catch(e){showError(e.message)}}});window.__bdhsGoogleInitialized=true}window.google.accounts.id.prompt()}catch(e){showError(e.message)}};
    }
